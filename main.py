...
def calculate_metrics(metrics):
    ...
    return {**metrics, 'roas': round(roas, 2), 'cpl': round(cpl, 2)}
...
def run_agent():
    ...
    for client_name, config in CLIENTS.items():
        print(f"📊 Analizando {client_name}...")
...
def schedule_jobs():
    ...
    scheduler.add_job(run_agent, 'cron', hour=7, minute=0)  # Cada día a las 7 AM
    scheduler.start()
    print("⏰ Scheduler iniciado - ejecutará a las 7 AM diariamente")
    try:
        while True:
            pass
